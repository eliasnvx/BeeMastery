...
public class FEItemScoop extends ItemForestry implements IColoredItem {
    ...
    public boolean isFoil(ItemStack stack) {
        return type == FEEnumScoop.LEGENDARY || type == FEEnumScoop.DRACONIC || super.isFoil(stack);
    }

    @Override
    public float getDestroySpeed(ItemStack itemstack, BlockState state) {
        if (state.is(ForestryTags.Blocks.MINEABLE_SCOOP)) {
            return 5.0F * type.miningSpeed;
        } else {
            return 1.0F;
        }
    }
    ...
    public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity interactionTarget, InteractionHand usedHand) {
        ...
        if (!level.isClientSide()) {
            if (interactionTarget instanceof Bee) {
                ItemEntity bee = new ItemEntity(level, interactionTarget.getX(), interactionTarget.getY(), interactionTarget.getZ(), 
                    SpeciesUtil.BEE_TYPE.get().createStack(ForestryBeeSpecies.FOREST, BeeLifeStage.DRONE));
