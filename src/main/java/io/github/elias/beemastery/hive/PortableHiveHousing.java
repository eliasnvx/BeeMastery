 ...
 *
 * <p>Forestry 2.5 chains housing modifiers multiplicatively (each modifier gets the previous one's
 * result, starting from the queen's speed gene) — the same as Forestry 3.x, so tier x booster
 * multiply exactly as in the 1.21.1 version. Its modifier interface has no flower or climate hook,
 * so the flower slot and the climate module waive the matching errors instead ({@link #waives}).
 */
public class PortableHiveHousing extends PlayerBeeHousing {
    ...
    /**
     * What Forestry 3.x's {@code providesFlowers} / {@code isClimateFullyTolerant} hooks do, for
     * Forestry 2.5: a suitable flower carried in the hive replaces the flowers Forestry looks for
     * around it, and the climate module lifts the temperature/humidity errors of the queen.
     */
    @Override
    protected boolean waives(IError error) {
        if (error == ForestryError.NO_FLOWER) {
            return HiveFlowers.suits(inventory.getFlower(), BeeStacks.genome(inventory.getQueen()));
        }
        if (error == ForestryError.TOO_HOT || error == ForestryError.TOO_COLD
                || error == ForestryError.TOO_HUMID || error == ForestryError.TOO_ARID) {
