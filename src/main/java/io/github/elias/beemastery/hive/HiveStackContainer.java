...
/**
 * The portable hive's inventory, living inside the hive item's {@code hive_contents} component.
 *
...
public class HiveStackContainer implements Container, IBeeHousingInventory {
    ...
    private NonNullList<ItemStack> load() {
        ...
        hive.getOrDefault(ModDataComponents.HIVE_CONTENTS.get(), ItemContainerContents.EMPTY).copyInto(items);
        return items;
    }

    private void store(NonNullList<ItemStack> items) {
        hive.set(ModDataComponents.HIVE_CONTENTS.get(), ItemContainerContents.fromItems(items));
    }
