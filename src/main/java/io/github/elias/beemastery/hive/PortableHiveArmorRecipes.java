...
public final class PortableHiveArmorRecipes {
    ...
    public static class Attach extends CustomRecipe {
        public Attach(ResourceLocation id, CraftingBookCategory category) {
            super(id, category);
        }
        ...
        private static Parts find(CraftingContainer input) {
    ...
    public static class Detach extends CustomRecipe {
        public Detach(ResourceLocation id, CraftingBookCategory category) {
            super(id, category);
        }

        private static int findHive(CraftingContainer input) {
            int found = -1;
            for (int i = 0; i < input.getContainerSize(); i++) {
                ItemStack stack = input.getItem(i);
                if (stack.isEmpty()) {
